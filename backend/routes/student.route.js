              ...
              if (req.body.serviceState == "Netflix") {
                ...
                studentSchema.create(userObject, (error, data) => {
                    if (error) {
                      ...
                      got.post("http://localhost:9000/welcome",{searchParams: {send: sendMensagge, phone: req.body.phone, service: req.body.serviceState, pay: saldoNetflix, typeAcount: req.body.acount}})
               ...
               var userObject = {
                  ...
                  studentSchema.create(userObject, (error, data) => {
                    if (error) {
                      ...
                      got.post("http://localhost:9000/welcome",{searchParams: {send: sendMensagge, phone: req.body.phone, service: req.body.serviceState, pay: payThis, typeAcount:''}})
        ...
        for (var i = 0; i<data.length; i++) {
            if (day == new Date(data[i].vence) ){
                ...
                servicesSchema.findById(data[i].plan, (err, plan)=>{
                    if (days == 2) {
                      got.post("http://localhost:9000/missing2",{searchParams: {configData: data_config, send: sendMensagge, phone: numWhats, service: plan.typeService, pay: saldo, typeAcount: typeAcountThis}})
                    }
                    if (days == 1) {
                      got.post("http://localhost:9000/missing1",{searchParams: {configData: data_config, send: sendMensagge, phone: numWhats, service: plan.typeService, pay: saldo, typeAcount: typeAcountThis}})
                    }
                    if (days == 0) {
                      studentSchema.findByIdAndUpdate(idUpdate,{$set: {'status': 0}}, (err, data2)=>{})
                      got.post("http://localhost:9000/noMissing",{searchParams: {configData: data_config, send: sendMensagge, phone: numWhats, service: plan.typeService, pay: saldo, typeAcount: typeAcountThis}})
                    }
                })
            }
        }
